...
exports.main = async (event, context) => {
  ...
  tasks.push(db.collection(`SportsMeet2021TransactionLog${event.logGrade}`).where({
  ...
  let checkAdminStatus = await db.collection("SportsMeet2021Admin").where({
    adminId: userId,
  }).get();
  ...
  if (canDeleteAll || logIssuer === userId) {
    await db.collection(`SportsMeet2021TransactionLog${event.logGrade}`).where({
      _id: event.deleteLogId,
    }).remove();
    return {
      status: "success"
