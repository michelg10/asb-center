...
exports.main = async (event, context) => {
  ...
  for (let i=0;i<grades.length;i++) {
    tasks.push(cloud.callFunction({
      name: "fetchAllCollections",
      data: {
        collectionName: `SportsMeetLeaderboardProcessed${grades[i]}`,
      }
    }))
  }
  for (let i=0;i<grades.length;i++) {
  ...
  for (let i=0;i<grades.length;i++) {
    for (let j=0;j<gradesComputedHomeroomList[i].length;j++) {
      ...
      tasks.push(db.collection(`SportsMeetHomeroomProcessed${grades[i]}`).doc(gradesComputedHomeroomList[i][j]._id).update({
        data: leaderboardUpdateObj
      }));
    }
  }
