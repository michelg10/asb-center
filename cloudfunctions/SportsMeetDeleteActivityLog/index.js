...
exports.main = async (event, context) => {
  ...
  tasks.push(db.collection(`SportsMeetStampLog${event.logGrade}`).where({
  ...
  if (logLeaderboardNumber !== undefined) {
    tasks.push(cloud.callFunction({
      ...
      data: {
        collectionName: `SportsMeetStampLog${event.logGrade}`,
        whereClause: {
          userId: userId,
          eventId: eventId
        }
  ...
  if (canDeleteAll || logIssuer === issuerUserId) {
    ...
    if (logStampNumber !== undefined) {
      tasks.push(db.collection(`SportsMeetHomeroomProcessed${event.logGrade}`).where({
    ...
    if (logLeaderboardNumber !== undefined) {
      ...
      tasks.push(db.collection(`SportsMeetLeaderboardProcessed${event.logGrade}`).where({
        studentId: userId,
      }).update({
        data: updateDataObj,
      }));
