...
exports.main = async (event, context) => {
  ...
  for (let i=0;i<event.userIds.length;i++) {
    console.log("Adding to ",`SportsMeet2021StampLog${userIdToGradeCache.get(event.userIds[i])}`);
    tasks.push(db.collection(`SportsMeet2021StampLog${userIdToGradeCache.get(event.userIds[i])}`).add({
      data: {
