    ...
    methods: {
      ...
      onShow: async function(){
        ...
        if(this.data.userData && this.data.userData.student){
          let checkHouse = await this.data.db.collection("BlackoutStudentData").where({
            userId: this.data.userData.student?.id,
          }).get();
      ...
      onLoad: async function(){
        ...
        eventChannel.on('userData', async (res) => {
          ...
          if(checkMeal.data.length===0){
            ...
            if(checkMeal.data[0].consent===true && this.data.consent===true){
              wx.navigateBack();
            }
      ...
      displayStudentsInSameHouse: async function() {
        try {
          let studentData = await this.data.db.collection("BlackoutStudentData").where({
            userId: this.data.userData.student?.id,
          }).get();
          let houseNumber = studentData.data[0].house;
          let houseStudents = await this.data.db.collection("BlackoutStudentData").where({
            house: houseNumber,
          }).get();
